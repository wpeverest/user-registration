...
class EmailService
{
	...
	public function send_user_backend_register_email($data)
	{
		...
		$subject          = __('Congratulations! Registration completed', 'user-registration-membership');
		$message          = sprintf(__('Hi, %s', 'user-registration-membership'), $data['username']) . "\n\n";
		$message         .= sprintf(__('Account created successfully under membership <b><i>%s</i></b>.', 'user-registration-membership'), $membership_title) . "\n\n";
		$extra_message    = sprintf(__('<b>Username: </b><i>%1$s</i> <b>Password: </b><i>%2$s</i> ', 'user-registration-membership'), $data['username'], $data['password']) . "\n\n";
		$final_greeting   = __('Thank You.', 'user-registration-membership');
	...
	public function send_user_register_email($data)
	{
		...
		$subject          = __('Congratulations! Registration completed', 'user-registration-membership');
		$message          = sprintf(__('Hi, %s', 'user-registration-membership'), $data['username']) . "\n\n";
		$message         .= sprintf(__('We have successfully received your request for registration under membership <b><i>%s</i></b>.', 'user-registration-membership'), $membership_title) . "\n\n";
		$extra_message    = '';
		$final_greeting   = __('Thank You.', 'user-registration-membership');

		if ('free' !== $data['payment_method']) {
			$message .= sprintf(__('Please wait for your payment to be verified. ', 'user-registration-membership'), $membership_title) . "\n\n";
		}
		$template_file = locate_template('membership-registration-email.php');
		if (!$template_file) {
			$template_file = UR_MEMBERSHIP_DIR . 'includes/Templates/Emails/membership-registration-email.php';
	...
	{
		$membership_title = get_the_title($data['membership']);
		$subject          = __('New Member Registration!', 'user-registration-membership');
		$message          = sprintf(__('A new user <b><i>%1$s</i></b> has been registered under membership <b><i>%2$s</i></b>,', 'user-registration-membership'), $data['username'], $membership_title) . "\n\n";
		$extra_message    = '';
		$final_greeting   = __('Thank You.', 'user-registration-membership');

		if ('free' !== $data['payment_method']) {
			$message .= sprintf(__('paid with <b><i>%s</i></b>.', 'user-registration-membership'), ucfirst($data['payment_method'])) . "\n\n";
		}
		$template_file = locate_template('membership-registration-email.php');
		if (!$template_file) {
			$template_file = UR_MEMBERSHIP_DIR . 'includes/Templates/Emails/membership-registration-email.php';
	...
	public function send_payment_successful_email($data)
	{
		...
		$subject          = __('Payment Received!', 'user-registration-membership');
		...
		$invoice_details  = array(
			...
			'billing_cycle'     => ('day' === $subscription['billing_cycle']) ? esc_html('Daily', 'user-registration-membership') : (esc_html(ucfirst($subscription['billing_cycle'] . 'ly'))),
	...
	public function send_payment_approval_email($data)
	{
		...
		$subject          = __('Payment Approved!', 'user-registration-membership');
		$currency         = get_option('user_registration_payment_currency', 'USD');
		$currencies       = ur_membership_get_currencies();
		$symbol           = $currencies[$currency]['symbol'];
		$message          = sprintf(__('Hi <b><i>%s</i></b>, Your payment of amount %s for the membership: <b>%s</b> has been approved by admin.', 'user-registration-membership'), $data['display_name'] ?? '', number_format($data['total_amount'], 2) . $symbol, $data['post_title'] ?? '') . "\n\n";
	...
	public function send_membership_cancellation_email_user($data)
	{
		...
		$subject = __('Membership Cancelled Successfully!', 'user-registration-membership');
		$user    = get_userdata($data['member_id']);

		$message        = sprintf(__('Hi <b><i>%1$s</i></b>, Your request for cancellation of current membership  <b><i>%2$s</i></b> was successful.', 'user-registration-membership'), esc_html($user->user_login), esc_html($data['membership_metas']['post_title'])) . "\n\n";
		$extra_message  = sprintf(__('We are sorry to see you leave.', 'user-registration-membership'), esc_html($user->user_login), esc_html($data['membership_metas']['post_title'])) . "\n\n";
		$final_greeting = __('Good Bye.', 'user-registration-membership');
		$template_file  = locate_template('membership-registration-email.php');
		if (!$template_file) {
			$template_file = UR_MEMBERSHIP_DIR . 'includes/Templates/Emails/membership-registration-email.php';
		}
	...
	public function send_membership_cancellation_email_admin($data)
	{
		...
		$subject = __('Membership Cancelled!', 'user-registration-membership');
		$user    = get_userdata($data['member_id']);

		$message        = sprintf(__('User <b><i>%1$s</i></b>, has cancelled their subscription under membership  <b><i>%2$s</i></b>.', 'user-registration-membership'), esc_html($user->user_login), esc_html($data['membership_metas']['post_title'])) . "\n\n";
		$extra_message  = '';
		$final_greeting = '';
		$template_file  = locate_template('membership-registration-email.php');
		if (!$template_file) {

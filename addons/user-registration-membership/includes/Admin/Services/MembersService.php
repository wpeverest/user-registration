	...
	public function validate_user_data( $data ) {
		...
		if ( username_exists( $data['username'] ) ) {
			return array(
				'status'  => false,
				'key'     => 'name',
				'message' => esc_html__( 'Username: ' . $data['username'] . ' is already taken.', 'user-registration-membership' ),
			);
		}
		// validate email
		if ( email_exists( $data['email'] ) ) {
			return array(
				'status'  => false,
				'key'     => 'name',
				'message' => __( 'Email: ' . $data['email'] . ' is already taken.', 'user-registration-membership' ),
			);
		}
		...
		if ( ! preg_match( '/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\w\s]).*$/', $data['password'] ) ) {
			return array(
				'status'  => false,
				'key'     => 'password',
				'message' => __( 'Password must contain at least one lowercase letter, one uppercase letter, one number, and one special character', 'user-registration-membership' ),
			);
		}
		// validate password
		if ( $data['password'] !== $data['confirm_password'] ) {
			if ( empty( $data['confirm_password'] ) ) {
				return array(
					'status'  => false,
					'message' => __( 'Confirm password field cannot be empty.', 'user-registration-membership' ),
				);
			}
			...
			return array(
				'status'  => false,
				'key'     => 'password',
				'message' => __( 'Password does not match.', 'user-registration-membership' ),
			);
		}
		...
		if ( isset( $data['coupon'] ) && ! empty( $data['coupon'] ) && ur_pro_is_coupons_addon_activated() ) {
			...
			if ( empty( $coupon_details ) ) {
				return array(
					'status'  => false,
					'key'     => 'coupon',
					'message' => __( 'Coupon does not exist.', 'user-registration-membership' ),
				);
			}
			if ( isset( $coupon_details['coupon_status'] ) && ! $coupon_details['coupon_status'] ) {
				return array(
					'status'  => false,
					'key'     => 'coupon',
					'message' => __( 'Coupon is Inactive.', 'user-registration-membership' ),
				);
			}
			if ( isset( $coupon_details['coupon_end_date'] ) && $coupon_details['coupon_end_date'] < date( 'Y-m-d' ) ) {
				return array(
					'status'  => false,
					'key'     => 'coupon',
					'message' => __( 'Coupon expired.', 'user-registration-membership' ),
				);
			}
			if ( 'membership' !== $coupon_details['coupon_for'] ) {
				return array(
					'status'  => false,
					'key'     => 'coupon',
					'message' => __( 'Invalid coupon type.', 'user-registration-membership' ),
				);
			}
			...
			if ( ! in_array( $data['membership'], $coupon_membership ) ) {
				return array(
					'status'  => false,
					'key'     => 'coupon',
					'message' => __( 'Coupon cannot be applied for the selected membership.', 'user-registration-membership' ),
				);
			}
			if ( $coupon_details['coupon_start_date'] > date( 'Y-m-d' ) ) {
				return array(
					'status'  => false,
					'key'     => 'coupon',
					'message' => __( 'Coupon is not valid until ' . date_i18n( get_option( 'date_format' ), strtotime( $coupon_details['coupon_start_date'] ) ) . '.', 'user-registration-membership' ),
				);
			}
			...
			if ( 'free' === $membership_meta['type'] ) {
				return array(
					'status'  => false,
					'key'     => 'coupon',
					'message' => __( 'Invalid membership type (Free).', 'user-registration-membership' ),
				);
			}
		}

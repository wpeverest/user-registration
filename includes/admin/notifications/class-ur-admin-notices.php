	...
	public static function continue_setup_wizard_notice() {
		...
		if ( ! $first_time_activation ) {
			...
			if ( $onboard_skipped && $onboard_skipped_step ) {
				...
				$onboarding_complete_text  = sprintf( __( '<a href="%s" class="button button-primary" style="margin-right: 5px;">Continue Setup Wizard</a>', 'user-registration' ), esc_url( admin_url( '/admin.php?page=user-registration-welcome&tab=setup-wizard&step=' . $onboard_skipped_step . '' ) ) );

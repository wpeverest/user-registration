...
class UR_Emailer {
	...
	private static function send_mail_to_admin( $user_email ) {
		...
		$message = apply_filters('user_registration_admin_email_message', __( sprintf( 'New user registered on <a href="%s">%s</a><br/>Email:%s', get_home_url(), $blog_info, $user_email ), 'user-registration' ) );

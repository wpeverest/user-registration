...
class UR_Log_Handler_File extends UR_Log_Handler {
	...
	public function clear( $handle ) {
		...
		do_action( 'user_registration_log_clear', $handle );
	...
	public static function get_log_file_path( $handle ) {
		if ( function_exists( 'wp_hash' ) ) {
			return trailingslashit( UR_LOG_DIR ) . sanitize_file_name( $handle . '-' . wp_hash( $handle ) . '.log' );
		} else {
			ur_doing_it_wrong( __METHOD__, __( 'This method should not be called before plugins_loaded.', 'user-registration' ), '3.0' );

		...
		switch ( $args['type'] ) {
			...
			if(isset($args['choices']) && count($args['choices'])>1 ){
				...
				foreach ( $choices as $choice_index => $choice ) {
					...
					$field .= '<label>';
					$field .= ' <input ' . implode( ' ', $custom_attributes ) . ' data-value="' . $choice_index . '" type="' . esc_attr( $args['type'] ) . '" class="input-checkbox ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" name="' . esc_attr( $key ) . '[]" id="' . esc_attr( $args['id'] ) . '_' . esc_attr( $choice_index ) . '" value="'.trim($choice).'"' . $value . ' /> ';
			...
			case 'select' :
				...
				if ( ! empty( $args['options'] ) ) {
					foreach ( $args['options'] as $option_key => $option_text ) {
						...
						$options .= '<option value="' . esc_attr( trim( $option_key ) ) . '" ' . selected( $value, $option_key, false ) . '>' . esc_attr( trim( $option_text ) ) . '</option>';
					}
			...
			case 'radio' :
				...
				if ( ! empty( $args['options'] ) ) {
					foreach ( $args['options'] as $option_key => $option_text ) {
						...
						$field .= '<input type="radio" class="input-radio ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" value="' . esc_attr( trim ( $option_key ) ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '_' . esc_attr( $option_key ) . '"' . checked( $value, $option_key, false ) . ' />' . wp_kses( trim( $option_text ), array(
								'a'    => array(
									'href'  => array(),
									'title' => array()
								),

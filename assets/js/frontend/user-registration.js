		...
		return this.each(function () {
			...
			var form = {
					...
					if (
						...
						$.each(frontend_field, function () {
							...
							if (single_field.length < 2) {
								...
								if (
									$.inArray(field_name, multi_value_field) < 0
								) {
				...
				get_fieldwise_data: function (field) {
					...
					$(
						document
					).trigger("user_registration_frontend_form_data_render", [
						field,
						formwise_data,
					]);
						...
						$registration_form
							...
							.on("submit", function (event) {
								...
								if (
									$(this)
										.closest("form")
										.find('input[name="ur-user-form-id"]')
										.length === 1
								) {
									form_id = $(this)
										.closest("form")

...
(function ($) {
	...
	if (_deprecated) {
		...
		var _before =
				'<button type="button" class="button wp-color-result" aria-expanded="false"></button>',
	...
	$.widget("wp.wpColorPicker", $.wp.wpColorPicker, {
		...
		_create: function () {
			...
			if (_deprecated) {
				el.wrap('<span class="wp-picker-input-wrap" />').after(
					self.button
				);
			} else {
				...
				change: function (event, ui) {
					if (self.options.alpha) {
						...
						if (_deprecated) {
							...
							if (
								self.toggler.find("span.color-alpha").length ==
								0
							) {
								self.toggler.append(
		...
		_addListeners: function () {
			...
			self.element.on("change", function (event) {
				...
				if (
					$(this).val() === "" ||
					self.element.hasClass("iris-error")
				) {
					if (self.options.alpha) {
						if (_deprecated) {
		...
		_create: function () {
			...
			if (
				...
				if (typeof el.data("custom-width") !== "undefined") {
					self.options.customWidth =
						parseInt(el.data("custom-width")) || 0;
				} else {
				...
				if (
					...
					el.width(
						parseInt(
							self.options.defaultWidth + self.options.customWidth
						)
					);

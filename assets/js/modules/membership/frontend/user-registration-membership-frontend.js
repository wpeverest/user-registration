...
(function ($, urmf_data) {
	...
	var ur_membership_ajax_utils = {
			...
			if (selected_team.length > 0) {
				...
				if ("variable" === seat_model) {
					...
					if ("tier" === pricing_model) {
						var selected_tier =
							selected_team.data("selected-tier");
				...
				if (
					...
					if (localCurrencyDetails[currency]) {
						...
						if ($membershipRadio.is(':checked')) {
							taxAmount = ur_membership_ajax_utils.calculate_tax_amount( newCalculatedValue );
							totalDetails.total = parseFloat( newCalculatedValue ) + parseFloat( taxAmount );
		...
		renew_membership: function (selected_pg, btn, membership_id, team_id) {
			this.send_data(
				{
	...
	var register_events = {
		init: function () {
			...
			$( document ).on( 'change', '.ur-field-address-country, .ur-field-address-state', function ( e ) {
				e.stopPropagation();
				e.preventDefault();
				var membership_input = $('input[name="urm_membership"]:checked');
			...
			$(".urm-team-pricing-tier").on("click", function () {
				...
				if (seat_model) {
					if ("fixed" === seat_model) {
						...
						var min_seats = parseInt(
							tier.data("minimum-seats"),
							10
						);
						var max_seats = parseInt(
							tier.data("maximum-seats"),
							10
						);
						if ("per_seat" === pricing_model) {

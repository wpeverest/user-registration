...
class StripeService {
	...
	public function process_stripe_payment( $payment_data, $response_data ) {
		...
		if ( ! empty( $local_currency ) && ! empty( $ur_zone_id ) && UR_PRO_ACTIVE && ur_check_module_activation( 'local-currency' ) && class_exists( CoreFunctions::class ) ) {
			...
			// Only switch to the local currency when the membership itself has local currency enabled.
			// Otherwise the amount stays in the global currency and switching the currency alone would
			// send an unconverted amount to Stripe (e.g. 20 USD charged as 20 NPR ≈ $0.13). Mirrors PayPal.

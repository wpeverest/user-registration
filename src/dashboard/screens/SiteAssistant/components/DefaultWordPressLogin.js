...
const DefaultWordPressLogin = ({ isOpen, onToggle, onHandled }) => {
	...
	const handleSave = async () => {
		...
		if (!isDisabled) {
			toast({
				...
				description: __('Please turn on the switch to disable WordPress login before saving.', 'user-registration'),
				status: 'warning',
				duration: 3000,
				isClosable: true,
			});
		...
		try {
			...
			if (result.success) {
				toast({
					...
					description: result.data?.message || __('WordPress login has been disabled successfully.', 'user-registration'),
					status: 'success',
					duration: 3000,
					isClosable: true,
				});
			...
			toast({
				...
				description: error.message || __('Failed to save settings. Please try again.', 'user-registration'),
				status: 'error',
				duration: 3000,
				isClosable: true,
			});
		...
		try {
			...
			if (result.success) {
				toast({
					...
					description: result.data?.message || __('WordPress login settings have been acknowledged and skipped.', 'user-registration'),
					status: 'success',
					duration: 3000,
					isClosable: true,
				});
		...
		>
			<HStack justify={'space-between'}>
				<Heading as="h3" size="md" fontWeight="semibold">
					{__('Default WordPress Login Enabled', 'user-registration')}
				</Heading>
				<IconButton
				...
				<Stack gap={5}>
					...
					>
						<Flex align="center" gap={2}>
							<Icon as={FaBookmark} w="4" h="4" color="orange.500" />
						...
						<Flex justify="space-between" align="center">
							<Box>
								<FormLabel fontSize="sm" fontWeight="medium" mb={1}>
									{__('Disable Default WordPress Login Screen', 'user-registration')}
								</FormLabel>
								<Text fontSize="xs" color="gray.600">
									{__('Prevents access to wp-login.php', 'user-registration')}
								</Text>
							</Box>
							<Switch

...
const PaymentSetup = ({ isOpen, onToggle, onSkipped }) => {
	const [isSkipping, setIsSkipping] = useState(false);
	const toast = useToast();

	// Get payment connections from backend
	const paymentConnections = window._UR_DASHBOARD_?.site_assistant_data?.payment_connections || {};

	// Check if any payment method is connected
	const hasConnectedPayment = Object.values(paymentConnections).some(connection => connection.is_connected);

	// Check if all payment methods are connected
	const allPaymentsConnected = Object.values(paymentConnections).every(connection => connection.is_connected);
	...
	const isPaymentSetupSkipped = window._UR_DASHBOARD_?.site_assistant_data?.payment_setup_handled === true && !hasConnectedPayment;
		...
		try {
			...
			if (result.success) {
				toast({
					...
					description: result.data?.message || __('Payment setup has been acknowledged and skipped.', 'user-registration'),
					status: 'success',
					duration: 3000,
					isClosable: true,
				});
				...
				throw new Error(result.data?.message || 'Failed to skip payment setup');
			}
		} catch (error) {
			toast({
				title: __('Error', 'user-registration'),
				description: error.message || __('Failed to skip payment setup. Please try again.', 'user-registration'),
				status: 'error',
				duration: 5000,
				isClosable: true,
			});
			...
			<Collapse in={isOpen}>
				<Stack gap={5}>
					...
					<VStack spacing={3} align="stretch">
						{Object.entries(paymentConnections).map(([key, connection]) => {
							const imageMap = {
								stripe: 'stripe.png',
								paypal: 'paypal.png',
								bank: 'bank.png',
							};
							const imageFile = imageMap[key] || 'payment.png';
							const assetsURL = window._UR_DASHBOARD_?.assetsURL || '';
								...
								>
									<Flex justify="space-between" align="center">
											...
											<HStack spacing={2} align="center">
												...
												{connection.is_connected && (
													<Badge
														...
														bg="green.50"
													>
														{__('Connected', 'user-registration')}
													</Badge>
												)}
											</HStack>
										</HStack>
										<Link
											...
											onClick={() => handleSettingsClick(connection.settings_url)}
											cursor="pointer"
					...
					{hasPaymentGateways && hasConnectedPayment && !allPaymentsConnected && (
						<Link
							color="primary.500"
							fontSize="sm"
							textDecoration="underline"

  ...
  const handleTemplateClick = async (template) => {
    ...
    try {
      ...
      toast({
        ...
        description: __("An error occurred while checking the plugin status. Please try again.", "user-registration"),
        status: "error",
  ...
  const handleFormTemplateSave = async () => {
    if (!formTemplateName) {
      toast({
        ...
        description: __("Please provide a name for your form.", "user-registration"),
        status: "warning",
        position: "bottom-right",
        duration: 5000,
        isClosable: true,
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast({
          title: __("Error", "user-registration"),
          description: response.message || __("Failed to create form template.", "user-registration"),
          status: "error",
          position: "bottom-right",
          duration: 5000,
          isClosable: true,
      ...
      toast({
        title: __("Error", "user-registration"),
        description: __("An error occurred while creating the form template.", "user-registration"),
        status: "error",
        position: "bottom-right",
        duration: 5000,
        isClosable: true,
  ...
  const mutation = useMutation(
    ...
    },
    {
      onError: (error) => {
        ...
        toast({
          title: __("Error", "user-registration"),
          description: __("An error occurred while updating favorites.", "user-registration"),
          status: "error",
          position: "bottom-right",
          duration: 5000,
          isClosable: true,
                  ...
                  {hoverCardId === template.id && (
                    ...
                    >
                      <Button borderRadius="50px" leftIcon={<IoPlayOutline />} colorScheme="purple" onClick={() => handleTemplateClick(template)}>
                        {__("Get Started", "user-registration")}
                      </Button>
...
/>
        ...
        >
          ...
          <Text margin={0} fontSize="sm" textAlign="center" color="gray.600">
            {__("Sorry, we didn't find any templates that match your criteria", "user-registration")}
          </Text>
        </Box>
      )}
			...
			<ModalContent borderRadius="8px" padding="20px">
				...
				<Heading  as="h2" margin="10px 0px 0px 0px" fontSize="20px" lineHeight="28px" fontWeight="bold">
  					{sprintf(__("%s is a Premium Template", "user-registration"), formTemplateName)}
				</Heading>
				</ModalHeader>
				...
				<Text margin="0px" fontSize="16px" lineHeight="24px" mb="20px">
					{__("This template requires premium addons. Please upgrade to the Premium to unlock all these awesome templates.", "user-registration")}
				</Text>
				</ModalBody>
	  ...
	  <ModalOverlay />
        <ModalContent  borderRadius="8px" padding="40px">
          <ModalHeader padding="0px" textAlign="left" fontSize="20px" lineHeight="28px" color="#26262E">
            {__("Uplift your form experience to the next level.","user-registration")}
          </ModalHeader>
          <ModalCloseButton top="12px" right="12px" />
          <ModalBody padding="0px" marginTop="16px">
            <Box mb="20px" padding="0px">
			<Text margin="0px 0px 6px" fontSize="16px" lineHeight="29px">{__("Give it a name","user-registration")}</Text>
              <Input
			  width={"full"}

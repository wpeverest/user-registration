      ...
      try {
        ...
        if (response.success) {
          ...
          toast({
            ...
            description: __("Invalid response format.", "user-registration"),
            status: "error",
        ...
        toast({
          title: __("Error", "user-registration"),
          description: __("Unable to check plugin status.", "user-registration"),
          status: "error",
          position: "bottom-right",
          duration: 5000,
          isClosable: true,
        ...
        for (const plugin of requiredPlugins) {
          try {
            ...
            if (response.success) {
              ...
              finalMessage = response.message || __("Plugin activated successfully.", "user-registration");
              ...
              finalMessage = response.message || sprintf(
                __("Failed to activate plugin: %s.", "user-registration"),
                plugin.value
              );
            }
          } catch (error) {
            ...
            finalMessage = sprintf(
              __("Unable to activate %s.", "user-registration"),
              plugin.value
            );
          }
        }

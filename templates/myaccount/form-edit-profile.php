									...
									if ( ! empty( $form_row_data ) ) {
										foreach ( $form_row_data as $key => $value ) {
											if ( $value['row_id'] == $row_id && isset( $value['type'] ) && 'repeater' === $value['type'] ) {
												$ignore              = true;
												$repeater_row_config = $value;
												break;
											}
										}
									}

									if ( $ignore && $repeater_row_config && ! empty( $repeater_row_config['field_name'] ) ) {
										$repeater_field_name = $repeater_row_config['field_name'];
										$repeater_id         = isset( $repeater_row_config['repeater_id'] ) ? $repeater_row_config['repeater_id'] : '';
										$repeater_row_title  = isset( $repeater_row_config['title'] ) && '' !== trim( (string) $repeater_row_config['title'] ) ? trim( $repeater_row_config['title'] ) : __( 'Repeater Row', 'user-registration' );
										$repeater_meta_key   = 'user_registration_' . $repeater_field_name;
										$repeater_values     = get_user_meta( $user->ID, $repeater_meta_key, true );
										$repeater_values     = ! empty( $repeater_values ) ? maybe_unserialize( $repeater_values ) : array();
										$repeater_values     = is_array( $repeater_values ) ? $repeater_values : array();

										$field_labels = array();
										foreach ( $row_data as $grid_data ) {
											foreach ( $grid_data as $single_item ) {
												if ( isset( $single_item->general_setting->field_name ) ) {
													$field_labels[ $single_item->general_setting->field_name ] = isset( $single_item->general_setting->label ) ? $single_item->general_setting->label : $single_item->general_setting->field_name;
												}
											}
										}

										echo '<div class="ur-form-row ur-repeater-row" data-row-id="' . esc_attr( $row_id ) . '" data-repeater-id="' . esc_attr( $repeater_id ) . '" data-repeater-field-name="' . esc_attr( $repeater_field_name ) . '">';
										echo '<div class="ur-form-grid">';
										echo '<div class="ur-repeater-label" id="user_registration_repeater_row_title_' . esc_attr( $repeater_id ) . '"><label class="ur-label">' . esc_html( $repeater_row_title ) . '</label></div>';

										if ( ! empty( $repeater_values ) ) {
											$row_index = 0;
											foreach ( $repeater_values as $repeater_row_index => $fields ) {
												++$row_index;
												echo '<div class="ur-repeater-row-items" data-repeater-row="' . esc_attr( $repeater_row_index ) . '">';
												$row_fields = is_array( $fields ) ? $fields : ( is_object( $fields ) ? (array) $fields : array() );
												foreach ( $row_fields as $field ) {
													$field_obj   = is_object( $field ) ? $field : (object) $field;
													$field_name  = isset( $field_obj->field_name ) ? $field_obj->field_name : '';
													$field_value = isset( $field_obj->value ) ? $field_obj->value : '';
													$field_type  = isset( $field_obj->field_type ) ? $field_obj->field_type : '';
													if ( '' === $field_name ) {
														continue;
													}
													$extra_params = isset( $field_obj->extra_params ) ? (array) $field_obj->extra_params : array();
													$label        = isset( $extra_params['label'] ) && '' !== trim( (string) $extra_params['label'] ) ? trim( $extra_params['label'] ) : ( isset( $field_labels[ $field_name ] ) ? $field_labels[ $field_name ] : $field_name );
													if ( in_array( $field_type, array( 'checkbox', 'multi-select' ), true ) && ! is_array( $field_value ) ) {
													...
													echo '<label class="ur-label">' . esc_html( $label ) . '</label>';
													if ( count( $repeater_values ) > 1 ) {
														echo '<div class="ur-repeater-row-title" style="font-size:12px; margin-bottom:5px;"><strong>' . sprintf( esc_html__( '( Row %d )', 'user-registration' ), $row_index ) . '</strong></div>';
													}
													echo '</div>';
													if ( 60 > strlen( (string) $field_value ) ) {
														printf( '<input type="text" value="%s" disabled>', esc_attr( $field_value ) );
													} else {
														printf( '<textarea rows="6" disabled>%s</textarea>', esc_textarea( $field_value ) );
													}
													echo '</div>';
												}
												echo '</div>';
											...
											echo '<div class="ur-repeater-row-items ur-repeater-empty"><p class="ur-description">' . esc_html__( 'No data.', 'user-registration' ) . '</p></div>';
										}
										echo '</div>';
										echo '</div>';
									} elseif ( ! $ignore ) {
